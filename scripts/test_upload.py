"""
Test script to verify evidence upload is working
"""
import requests
import os

API_BASE = "https://margarakshak-backend.onrender.com"

print("=" * 80)
print("Testing Evidence Upload System")
print("=" * 80)

# Test 1: Check if upload endpoint exists
print("\n1. Testing upload endpoint availability...")
try:
    # Try to upload a test file
    test_image_path = "test_image.jpg"
    
    # Create a small test image (1x1 pixel JPEG)
    # Minimal valid JPEG
    jpeg_header = bytes([
        
    ])
    
    with open(test_image_path, 'wb') as f:
        f.write(jpeg_header)
    
    print(f"✓ Created test image: {test_image_path}")
    
    # Get a report ID to test with
    import pymysql
    db_config = {
        'host': '127.0.0.1',
        'user': 'root',
        'password': 'yvpandi@11',
        'database': 'traffic_violation_db'
    }
    conn = pymysql.connect(**db_config)
    cursor = conn.cursor()
    cursor.execute("SELECT report_id FROM REPORTS ORDER BY report_id DESC LIMIT 1")
    result = cursor.fetchone()
    conn.close()
    
    if result:
        report_id = result[0]
        print(f"✓ Using latest report ID: {report_id}")
        
        # Try to upload
        with open(test_image_path, 'rb') as f:
            files = {'file': ('test.jpg', f, 'image/jpeg')}
            response = requests.post(
                f"{API_BASE}/api/reports/upload-evidence/{report_id}",
                files=files
            )
        
        print(f"\nUpload Response Status: {response.status_code}")
        print(f"Response: {response.json()}")
        
        if response.status_code == 200:
            print("\n✅ Upload successful!")
            print(f"Image saved at: {response.json().get('evidence_path')}")
        else:
            print(f"\n❌ Upload failed: {response.json()}")
    else:
        print("❌ No reports found in database")
    
    # Cleanup
    if os.path.exists(test_image_path):
        os.remove(test_image_path)
        print(f"\n✓ Cleaned up test image")
        
except Exception as e:
    print(f"\n❌ Error: {e}")
    import traceback
    traceback.print_exc()

print("\n" + "=" * 80)
print("Test Complete")
print("=" * 80)
