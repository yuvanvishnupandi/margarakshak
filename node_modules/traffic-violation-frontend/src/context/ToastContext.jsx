...
function ToastContainer({ toasts, onRemove }) {
  ...
  const getToastStyles = (type) => {
    switch (type) {
      ...
      case 'warning':
        return 'bg-yellow-50 border-yellow-500 text-yellow-800 dark:bg-yellow-900/30 dark:border-yellow-600 dark:text-yellow-200'
      case 'info':
  ...
  return (
    <div className="fixed top-4 right-4 z-50 space-y-2 max-w-sm">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={`flex items-center gap-3 px-4 py-3 border-l-4 rounded-lg shadow-lg animate-slide-in ${getToastStyles(toast.type)}`}
        >
          {getIcon(toast.type)}
