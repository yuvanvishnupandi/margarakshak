...
function SubmitReport() {
    ...
    if (name === 'location_address') {
      ...
      if (value.length > 3) {
        ...
        window._locTimer = setTimeout(async () => {
          try {
            const res = await fetch(`http://localhost:5000/api/weather/geocode?q=${encodeURIComponent(value)}`)
  ...
  const fetchVehicleInfo = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/weather/vehicle?plate=${plate}`)
      const data = await res.json()
    ...
    try {
      ...
      for (const img of evidenceImages) {
        try {
          ...
          fd.append('file', img)
          await fetch(`http://localhost:5000/api/reports/upload-evidence/${reportId}`, {
            method: 'POST', body: fd
          })
        } catch (err) {
