...
function SubmitReport() {
  ...
  const [aiAnalyzingCount, setAiAnalyzingCount] = useState(0)
  ...
  const verifyImageWithAI = async (base64String, file) => {
    ...
    try {
      ...
      const res = await fetch(`${AI_BASE_URL}/api/ai/process-evidence`, {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(errorMsg || 'AI Service Error')
      }
      
      if (data.is_fraud) {
        showError(`AI Rejected (Fraud Check): ${data.fraud_reason}`)
        setImagePreviews(prev => prev.filter(p => p !== base64String))
        setEvidenceImages(prev => prev.filter(f => f !== file))
        return
      }

      const validation = data.vision_validation
      if (validation) {
        if (validation.is_valid_submission) {
          setFormData(f => {
            if (f.plate_no && validation.extracted_plate && f.plate_no !== validation.extracted_plate) {
              setTimeout(() => {
                showError(`Conflict: Photo has plate ${validation.extracted_plate}, but report is for ${f.plate_no}. Only 1 vehicle allowed per report.`)
                setImagePreviews(prev => prev.filter(p => p !== base64String))
                setEvidenceImages(prev => prev.filter(fv => fv !== file))
              }, 0)
              return f
            }
            
            setTimeout(() => {
              success(`AI Verified: Found Plate ${validation.extracted_plate || 'N/A'}`)
              if (validation.extracted_plate) {
                const fetchBtn = document.getElementById('parivahan-fetch-btn');
            ...
            return {
              ...f,
              plate_no: validation.extracted_plate || f.plate_no,
              violation_type: validation.violation_detected || f.violation_type
            }
          })
        } else {
  ...
  const handleImageChange = (e) => {
    ...
    for (const file of toAdd) {
      ...
      if (evidenceImages.some(f => f.name === file.name && f.size === file.size) || validated.some(f => f.name === file.name && f.size === file.size)) {
        showError(`${file.name} is already added`); continue
      }
      validated.push(file)
    }
    ...
    validated.forEach((file, i) => {
      ...
      reader.onloadend = () => {
        ...
        // Trigger AI triage for EVERY newly added image
        verifyImageWithAI(reader.result, file)
      }
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
                      ...
                      <button type="button" id="parivahan-fetch-btn" onClick={fetchVehicleInfo} disabled={vehicleLoading} style={{
                    ...
                    {imagePreviews.length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', marginBottom: '12px' }}>
                        {imagePreviews.map((src, i) => (
                          <div key={i} className="photo-thumb-appear" style={{ position: 'relative', borderRadius: '10px', overflow: 'hidden', border: '2px solid #4f46e5', width: '150px', height: '112px', background: '#f5f3ff', flexShrink: 0 }}>
                  ...
                  <button
                    type="submit"
                    disabled={loading || aiAnalyzingCount > 0 || evidenceImages.length === 0 || !formData.plate_no}
                    style={{
                      width: '100%', 
                      padding: '16px', 
                      background: (loading || aiAnalyzingCount > 0 || evidenceImages.length === 0 || !formData.plate_no) ? '#e2e8f0' : '#4f46e5',
                      color: (loading || aiAnalyzingCount > 0 || evidenceImages.length === 0 || !formData.plate_no) ? '#64748b' : '#ffffff', 
                      border: 'none', 
                      borderRadius: '12px', 
                      fontSize: '16px', 
                      fontWeight: 700,
                      cursor: (loading || aiAnalyzingCount > 0 || evidenceImages.length === 0 || !formData.plate_no) ? 'not-allowed' : 'pointer', 
                      transition: 'all 0.2s ease',
                      display: 'flex', 
                      alignItems: 'center', 
                      justifyContent: 'center', 
                      gap: '8px',
                      boxShadow: (loading || aiAnalyzingCount > 0 || evidenceImages.length === 0 || !formData.plate_no) ? 'none' : '0 4px 12px rgba(79, 70, 229, 0.3)'
                    }}
                  >
                    {(loading || aiAnalyzingCount > 0) ? (
                      <>
                        ...
                        {aiAnalyzingCount > 0 ? 'AI Check In Progress...' : 'Submitting...'}
                      </>
                    ) : (
                      <>
                        ...
                        {(!formData.plate_no || evidenceImages.length === 0) ? 'Waiting for Valid Vehicle Photo' : 'Submit Violation Report'}
                      </>
                    )}
                  </button>
