...
function MyChallans({ user }) {
  ...
  useEffect(() => {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognition.onresult = (event) => {
        ...
        if (finalTranscript) {
          setDisputeReason((prev) => (prev + ' ' + finalTranscript).trim());
        }
      };
  ...
  const toggleVoiceListening = () => {
    ...
    if (isListening) {
      ...
      // AI greeting on first activation
      if (!disputeReason) {
        speakText("I am listening. Please explain why you want to dispute this challan.");
      }
    }
  };
