...
function ReviewReports() {
  ...
  const fetchReports = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/reports/police/pending`, { headers })
  ...
  const handleVerify = async (id) => {
    try {
      ...
      const lockRes = await fetch(`${API_BASE_URL}/api/reports/${id}/lock`, { method: 'POST', headers })
      if (!lockRes.ok) {
        const d = await lockRes.json()
        throw new Error(d.error || 'Failed to lock report')
      }
      navigate(`/police/create-challan/${id}`)
    } catch (err) {
      showError(err.message)
      fetchReports() // refresh queue
    }
  }
  ...
  const handleReject = async (id) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/reports/police/process/${id}`, { method:'PUT', headers, body:JSON.stringify({status:'Rejected',badge_no: actualBadge}) })
