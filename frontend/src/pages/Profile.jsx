...
function Profile({ user }) {
  ...
  const fetchProfileData = async () => {
    try {
      ...
      let reports = { reports: [] }
      try {
        const reportsRes = await fetch(`http://localhost:5000/api/reports/my-reports/${user?.id}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
        if (reportsRes.ok) {
      ...
      try {
        const challansRes = await fetch(`http://localhost:5000/api/challans/citizen/${user?.id}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
        if (challansRes.ok) {
